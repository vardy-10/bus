layui.use([ 'table' ], function() {
	...
	table.render({
		...
		cols : [ [ {
			...
			title : '公告内容',
			templet: function(d){	 
	    		  return HTMLEncode(d.notice_content);
	    	  }	
		},	 
		 {
			fixed : 'right',
	...
	table.on('tool(test)', function(obj) {
		...
		case 'update':
			...
			layer.open({
				type : 2,
				title : [ '修改通知公告信息', 'font-size:16px;' ],
				shadeClose : true,
				scrollbar : false,
				area : [ '100%', '100%' ],
				content : '/manage/noticeUpdate?release_time='+data.release_time+"&admin_id="+data.admin_id,
...
layui.use('form', function() {
	...
	$("#addNotice").click(function() {
		layer.open({
			type : 2,
			title : [ '添加通知公告', 'font-size:16px;' ],
			shadeClose : true,
			scrollbar : false,
			area : [ '100%', '100%' ],
			content : '/manage/noticeAdd',
			end : function() {
