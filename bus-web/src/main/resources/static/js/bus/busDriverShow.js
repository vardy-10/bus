layui.use([ 'table' ], function() {
	...
	table.render({
		...
		cols : [ [ {
			...
			width:'20%',
			templet: function(d){	
	    		  return HTMLEncode(d.driver_number);
	    	  }	 
		},  
		{
			/*field : 'driver_name',*/
	...
	table.on('tool(test)', function(obj) {
		...
		case 'update':
			...
			layer.open({
				type : 2,
				title : [ '编辑司机【'+HTMLEncode(data.driver_name)+'】', 'font-size:16px;' ],
				shadeClose : true,
				scrollbar : false,
				area : [ '100%', '100%' ],
				content : '/manage/busDriverUpdate?driver_id='+data.driver_id,
		...
		case 'delete':
			...
			console.log(data);
			// 询问框
			layer.confirm('确认删除【 ' + HTMLEncode(data.driver_name) + '】 吗？', {
				btn : [ '删除', '取消' ]
			// 按钮
			}, function() {
				//发送ajax请求
